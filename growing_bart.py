...
class SimpleGenerator(nn.Module):############################# màu hồng
...
class ParameterGenerator(nn.Module):############################# màu hồng
    def __init__(self, config):
        super().__init__()
...
class GrowingBart(nn.Module):
    ...
    def forward(self, rel_ids, rel_masks, input_ids, input_masks, output_ids, output_masks, is_training=False,dumble=None):###################################################### dumble
        # # generate adapter parameters using task descriptions
        # generated_params = self.meta_model(rel_ids, attention_mask=rel_masks)

        # # apply the parameters to the adapters
        # self.apply_params_to_adapters(generated_params)   ########################### hồng sang tím 
        ...
        if  CHECKPOINT == False:
            ...
            ret = torch.utils.checkpoint.checkpoint(self.model,input_ids, input_masks,None ,
                        output_ids,
                        output_masks, None, False ,
                        is_training, dumbledore                        
            )
        return ret    
